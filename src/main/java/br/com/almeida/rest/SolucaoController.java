...
import br.com.almeida.dao.SolucaoDao;
...
public class SolucaoController {
	...
	SolucaoDao solDao = new SolucaoDao();
	Solucao sol = null;
	String retorno = null;
	Gson gson = new Gson();
	...
	public Response buscaId(@PathParam("id") int id) {
		...
		if (id > 0) {
			...
			try {
				...
				sol = new Solucao();
				sol = (Solucao) solDao.buscaPorId(id);

				if (sol != null) {

					retorno = gson.toJson(sol);
	...
	public Response buscaNome(@PathParam("nome") String nome) {
		...
		if (nome.length() > 3) {
			listaObjeto = solDao.buscarPorNome(nome);
	...
	public Response incluir(String json) {
		...
		try {
			...
			System.out.println("dados a inserir: " + json);

			sol = gson.fromJson(json, Solucao.class);

			if (sol.getAssunto().length() > 3) {

				idInserido = solDao.incluir(sol); // Após incluir, recebo o novo ID incluso
	...
	@GET
	@Path("buscaidempresa/{id}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response buscaIdEmpresa(@PathParam("id") int id) {
		retorno = "";
		if (id > 0) {
			...
			try {
				...
				sol = new Solucao();
				sol = (Solucao) solDao.buscarPorEmpresa(id);
